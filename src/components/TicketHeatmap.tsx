...
export default function TicketHeatmap({ onCellClick }: Props) {
  ...
  const fetchData = useCallback(() => {
    ...
    api.get("/tickets/heatmap", { params })
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl mb-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-200">
        <h2 className="text-gray-900 font-semibold">Employee Ticket Summary</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-500 text-sm">No tickets for this period</div>
      ) : (
              ...
              <tr className="border-b border-gray-200">
                ...
                {ticketStatuses.map((s) => (
                  <th key={s} className="text-center px-3 py-3 text-gray-600 font-medium whitespace-nowrap">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs ${statusColors[s]}`}>{s}</span>
                  </th>
                ))}
                  ...
                  {ticketStatuses.map((s) => {
                    ...
                    return (
                      <td key={s} className="text-center px-3 py-2.5">
                        {count > 0 ? (
                          <button onClick={() => onCellClick(row.employee_id, s, period)}
                            className={`inline-block min-w-[32px] px-2 py-1 rounded text-xs font-semibold cursor-pointer hover:ring-2 hover:ring-blue-400 transition-all ${statusColors[s]}`}>
                            {count}
                          </button>
              ...
              <tr className="bg-gray-100 border-t border-gray-300">
                ...
                {ticketStatuses.map((s) => {
                  ...
                  return (
                    <td key={s} className="text-center px-3 py-2.5">
                      {count > 0 ? (
                        <button onClick={() => onCellClick(null, s, period)}
                          className={`inline-block min-w-[32px] px-2 py-1 rounded text-xs font-bold cursor-pointer hover:ring-2 hover:ring-blue-400 transition-all ${statusColors[s]}`}>
                          {count}
                        </button>
                      ) : <span className="text-gray-400 text-xs">0</span>}
                    </td>
